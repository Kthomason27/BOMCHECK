...
export default function NpdBomChecker() {
  ...
  const uniqueKits = React.useMemo(() => {
    ...
    comparisonTable.forEach((row) => {
      ...
      if (npdKit) kits.add(String(npdKit));
      if (bomKit) kits.add(String(bomKit));
    });
  ...
  const uniquePositions = React.useMemo(() => {
    ...
    comparisonTable.forEach((row) => {
      ...
      if (npdItem) positions.add(String(npdItem));
      if (bomItem) positions.add(String(bomItem));
    });
  ...
  const filteredTable = React.useMemo(() => {
    return comparisonTable.filter((row) => {
      // Kit filter
      if (kitFilter !== "all") {
        const npdKit = String(row.npd["Kit #"]);
        const bomKit = String(row.bom["Kit Item"]);
      ...
      if (positionFilter !== "all") {
        ...
        const bomItem = row.bom["Position"];
        if (
          String(npdItem) !== positionFilter &&
          String(bomItem) !== positionFilter
        ) {
          return false;
        }
      }
  ...
  const handleCompare = async () => {
    try {
      ...
      const matchKey = (row) =>
        `${row["Kit #"] || ""}|${row["Position #"] || ""}|${
          row["Item #"] || row["h2m Item #"] || ""
        }`;

      const bomMatchKey = (row) =>
        `${row["Kit Item"] || ""}|${row["Position"] || ""}|${
          row["Item number"] || ""
        }`;
